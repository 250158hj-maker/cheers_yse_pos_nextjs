...
<!-- 変更 2026-05-07 v1.2→v1.3 ER図確定（Q16/Q17/Q20/Q21反映） -->
**バージョン：** 1.3（Next.js版）
...
<!-- 追記 2026-05-07 v1.3 変更履歴 -->
### v1.3 変更履歴

- 概念設計：`sale_items` に `product_name` を明示し、売上時点のスナップショット保存を明文化（実装フェーズ Q16）
- 概念設計：`store_products` テーブルを削除（要件定義書および ER図と整合。実装フェーズ Q17）
- 概念設計：`product_categories` テーブルを削除し、`products` が `category_id` を直接保持する1対多構造に変更（要件定義書と整合。実装フェーズ Q21）
- 概念設計：`users` テーブル統合の根拠を明記。複数管理者アカウント管理がスコープ外のため `stores` と `admin` を統合し、`is_admin` フラグでロール分類（実装フェーズ Q20）
- 論理設計（ERD）：上記4点を mermaid 図に反映
...
<!-- 変更 2026-05-07 v1.3: store_products・product_categories 削除、users 統合根拠追記、products に category_id を持たせる構造に変更 -->

| エンティティ | 属性 | 備考 |
| --- | --- | --- |
| users | 名前・ログインID・パスワード・ロール | 認証用データ。店舗ごとに1アカウント + 管理者用のアカウント（`is_admin` フラグで分類）。複数管理者管理がスコープ外のため `stores` と `admin` を統合 |
| **products**（商品） | 商品名・価格・TO区分・カテゴリID | 商品マスタ。1商品が1カテゴリに属する（categories と1対多） |
...
<!-- 変更 2026-05-07 v1.3: 多対多（store_products / product_categories）の関連を削除、商品とカテゴリは1対多に -->

- カテゴリ 1対多 商品（1つのカテゴリには複数の商品が属する）
...
<!-- 変更 2026-05-07 v1.3: product_categories を削除、products に category_id を追加、sale_items に product_name を追加 -->
